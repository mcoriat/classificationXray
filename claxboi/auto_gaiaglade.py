...
# -*- coding: utf-8 -*-
"""
Enrich X-ray catalog with GLADE galaxy properties, Gaia proper motions, and distances.

Matches X-ray sources against the GLADE galaxy catalog (sky ellipse matching),
Gaia EDR3 proper motions (via NWAY results), and Gaia EDR3 distances.
Computes derived quantities: X-ray luminosity, galactic coordinates,
flux-to-magnitude ratios (logFxFb, logFxFr, logFxFw1, logFxFw2).
...
MPC_TO_CM = 3.086e24   # 1 Mpc in cm
PC_TO_CM = 3.086e18    # 1 pc in cm
...
if __name__ == "__main__":
    ...
    if not(skip_glade):    
        ...
        res.add_column(np.asarray(res[fx_c1])*4*np.pi*(np.asarray(res['Dist'])*MPC_TO_CM)**2,name="Lx_1")
