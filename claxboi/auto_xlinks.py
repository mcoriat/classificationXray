...
# -*- coding: utf-8 -*-
"""
Cross-link X-ray detections across XMM-Newton, Swift/LSXPS, and Chandra CSC2 catalogs.

Identifies common sources, assigns a unique MASTER_ID, and computes
maximum-to-minimum flux ratios. Uses STILTS for positional cross-matching
at 1-sigma and 3-sigma position errors.
...
if __name__ == "__main__":
    ...
    for names in []:#"4XMM_DR12 CSC2", "2SXPS CSC2", "4XMM_DR12 2SXPS"]:
        ...
        cmd = ['stilts', 'tmatch2', 'matcher=skyerr', 'find=all', 'join=1and2',
               'out=match3sig_%s_%s.fits' % (name1, name2),
               'in1=%s_udet_thin.fits' % name1, 'in2=%s_udet_thin.fits' % name2,
               'params=1', 'values1=ra dec 3*err63', 'values2=ra dec 3*err63']
        print(' '.join(cmd))
        subprocess.run(cmd, check=True)

        cmd = ['stilts', 'tmatch2', 'matcher=skyerr', 'find=all', 'join=1and2',
               'out=match1sig_%s_%s.fits' % (name1, name2),
               'in1=%s_udet_thin.fits' % name1, 'in2=%s_udet_thin.fits' % name2,
               'params=1', 'values1=ra dec err63', 'values2=ra dec err63']
        print(' '.join(cmd))
        subprocess.run(cmd, check=True)
